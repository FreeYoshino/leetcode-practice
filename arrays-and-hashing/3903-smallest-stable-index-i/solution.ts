/**
 * 題號：3903
 * 題目：Smallest Stable Index I
 * 連結：https://leetcode.com/problems/smallest-stable-index-i/description/
 * 時間複雜度：O(n)
...
function firstStableIndex(nums: number[], k: number): number {
  ...
  // maxArray 從左到右，紀錄每個位置的最大值
  // minArray 從右到左，紀錄每個位置的最小值
  for (let i = 1; i < n; i++) {
  ...
  for (let i = 0; i < n; i++) {
    const score = maxArray[i] - minArray[i];
